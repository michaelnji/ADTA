...
const numToArray = computed(() => {
    const formattedNum = Intl.NumberFormat('cm').format(props.amount).toString()
    return formattedNum.split(',')
})
console.log(numToArray.value)
</script>
<template>
    <div v-if="!format">
        <ClientOnly>
            <layflags-rolling-number style="--roll-duration:750ms" :value="amount"></layflags-rolling-number>
        </ClientOnly>
    </div>
